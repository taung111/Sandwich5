...
def update_recipe(recipe_id: int, recipe: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    db_recipe = recipes.update(db=db, recipe=recipe, recipe_id=recipe_id)
...
def delete_recipe(recipe_id: int, db: Session = Depends(get_db)):
    recipe = recipes.delete(db=db, recipe_id=recipe_id)
    if recipe is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
...
def read_order_details(db: Session = Depends(get_db)):
    return order_details.read(db=db)
...
def update_order_detail(order_detail_id: int, order_detail_update: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    db_order_detail = order_details.update(db, order_detail_id, order_detail_update)
    if db_order_detail is None:
        raise HTTPException(status_code=404, detail="OrderDetail not found")
    return db_order_detail

@app.delete("/order_details/{order_detail_id}", response_model=schemas.OrderDetail, tags=["Order Details"])
def delete_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    db_order_detail = order_details.delete(db, order_detail_id)
    if db_order_detail is None:
        raise HTTPException(status_code=404, detail="OrderDetail not found")
    return db_order_detail